...
/** Upsert an array of offenses in a single transaction, then update the status count (admin-only endpoint). */
export async function bulkUpload(req: Request, res: Response) {
  ...
  try {
    ...
    const { rows } = await client.query("SELECT COUNT(*)::int AS count FROM offenses");
    await client.query("UPDATE status SET total_offenses = $1, last_updated = NOW() WHERE id = 1", [rows[0].count]);
